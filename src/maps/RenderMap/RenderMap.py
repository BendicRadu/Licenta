...
class RenderMap:
    ...
    def get_player_auto_move(self):
        ...
        if tile_under_player not in GameVars.TILES_MOVERS_DIRECTIONS.keys():
            return None

        direction = GameVars.TILES_MOVERS_DIRECTIONS[tile_under_player]
        speed     = GameVars.TILES_MOVERS_SPEEDS[tile_under_player]
    ...
    def validate_direction(self, direction):
        ...
        tile_top_right = self.screen_map.get_tile_from_screen(
            GameVars.PLAYER_SCREEN_X + GameVars.PLAYER_SIZE,
            GameVars.PLAYER_SCREEN_Y,
            cam_x, cam_y
        )
        ...
        tile_bottom_left = self.screen_map.get_tile_from_screen(
            GameVars.PLAYER_SCREEN_X,
            GameVars.PLAYER_SCREEN_Y + GameVars.PLAYER_SIZE,
            cam_x, cam_y
        )
        ...
        if tile_top_left.tile_code in GameVars.TILES_WITH_COLLIDERS:
            if player_rect.colliderect(tile_rect_top_left):
                return False

        if tile_bottom_right.tile_code in GameVars.TILES_WITH_COLLIDERS:
            if player_rect.colliderect(tile_rect_bottom_right):
                return False


        if tile_top_right.tile_code in GameVars.TILES_WITH_COLLIDERS:
            if player_rect.colliderect(tile_rect_top_right):
                return False

        if tile_bottom_left.tile_code in GameVars.TILES_WITH_COLLIDERS:
    ...
    def to_rect(self, tile, cam_x, cam_y):
        ...
        x = tile.j * GameVars.TILE_SIZE - cam_x
        y = tile.i * GameVars.TILE_SIZE - cam_y

        return pygame.Rect(x, y, GameVars.TILE_SIZE, GameVars.TILE_SIZE)
    ...
    def get_selected_tile_sprite(self, mouse_pos_raw):
        ...
        x = tile.j * GameVars.TILE_SIZE - self.camera.offset_x
        y = tile.i * GameVars.TILE_SIZE - self.camera.offset_y
    ...
    def get_tile_under_player(self):
        player_pos = self.unapply_camera((GameVars.GAME_SCREEN_CENTER_X_RAW, GameVars.GAME_SCREEN_CENTER_Y_RAW))
        return self.screen_map.get_selected_tile(player_pos)


    def remove_selected_tile(self, mouse_pos_raw):
        mouse_pos = self.unapply_camera(mouse_pos_raw)
        self.screen_map.update_selected_tile(mouse_pos, GameVars.TileCode.GRASS.value)
    ...
    def get_neighbour_moves(self, tile_1, tile_2):
        ...
        base_direction = (
            (tile_2[1] - tile_1[1]) * GameVars.PLAYER_SPEED,
            (tile_2[0] - tile_1[0]) * GameVars.PLAYER_SPEED
        )

        repeat = GameVars.TILE_SIZE // GameVars.PLAYER_SPEED
    ...
    def get_move_to_center_moves(self):
        ...
        player_pos_raw = self.unapply_camera((GameVars.GAME_SCREEN_CENTER_X, GameVars.GAME_SCREEN_CENTER_Y))

        # player position inside the tile
        cam_x = player_pos_raw[0] % GameVars.TILE_SIZE
        cam_y = player_pos_raw[1] % GameVars.TILE_SIZE

        tile_center = GameVars.TILE_SIZE // 2

        tile_min = tile_center - GameVars.PLAYER_SPEED // 2
        tile_max = tile_center + GameVars.PLAYER_SPEED // 2

        while tile_min > cam_x:
            cam_x += GameVars.PLAYER_SPEED
            cam_x %= GameVars.TILE_SIZE
            moves.append((GameVars.PLAYER_SPEED, 0))
        ...
        while tile_min > cam_y:
            cam_y += GameVars.PLAYER_SPEED
            moves.append((0, GameVars.PLAYER_SPEED))
...
class Camera:
    ...
    def move_camera(self, direction):
        ...
        self.offset_x %= GameVars.TILE_SIZE
        self.offset_y %= GameVars.TILE_SIZE
